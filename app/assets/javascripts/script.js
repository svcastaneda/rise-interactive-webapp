$(document).ready(function() {
	...
	$(".gaStuff, #gaAccountInfo, .adobeStuff, #hasAdobeAccess, .tagManager, .GTM, .DTM, #enhancedEcommerce, #benefits, #hasTnO, #hasVOC, #isVOCpossible, #callTrackingBenefits, #hasGoogleAccess, [name^=GoogleHasGoals], [name^=GoogleHasFilters], [name^=GoogleHasEvents], [name^=AdobeHasGoals], [name^=AdobeHasFilters], [name^=AdobeHasEvents], .hasTagManager, .recommendedTagManager, .whyThisTM").hide();
	...
	$("[name^=tagManager]").change(function() {
		if (this.value != "no tag manager") {
			$(".hasTagManager").fadeIn("slow");
		} else {
			$(".hasTagManager").fadeOut("slow");
			$(".recommendedTagManager").fadeIn("slow");
		};
	});

	$("#otherTagManagerInput").focus(function() {
		$("[name^=tagManager]:focus").val("other");
		$("#otherTagManager").attr('checked',true);
	});
	
	$(".hasTagManager").change(function() {
		if ($("[name^=recommendTM]:checked").val() == 'yes') {
			$(".recommendedTagManager").fadeIn("slow");
		} else {
			$(".recommendedTagManager, .whyThisTM").fadeOut("slow");
		};
	});
	
	$(".recommendedTagManager").change(function() {
		if ($("[name^=recommendedTagManager]:checked").val() == 'other') {
			$(".whyThisTM").fadeIn("slow");
		} else {
			$(".whyThisTM").fadeOut("slow");
		};
	});
